...
class AccountView(arcade.View):
    ...
    def _build_buttons(self):
        ...
        y_values = (180, 135, 78) if len(data) == 3 else (155, 95)
        return [
    ...
    def _draw_form(self) -> None:
        ...
        arcade.draw_text("CREATE A NEW ACCOUNT" if is_register else "SIGN IN TO YOUR ACCOUNT",
    ...
    def on_mouse_press(self, x, y, button, modifiers) -> None:
        ...
        if not account:
            for i, rect in enumerate(self._field_rects):
                if self._mode != "register" and i == 2:
                    continue
    ...
    def on_key_press(self, key, modifiers) -> None:
        ...
        if key in (arcade.key.TAB, arcade.key.DOWN, arcade.key.RIGHT):
            max_field = 2 if self._mode == "register" else 1
    ...
    def on_text(self, text: str) -> None:
        if self._submitting or leaderboard_client.current_account() or not text.isprintable():
            return
        if self._selected_field == 0 and len(self._email) < 254:
            self._email += text
        elif self._selected_field == 1 and len(self._password) < 128:
            self._password += text
        elif self._selected_field == 2 and len(self._player_name) < 20:
